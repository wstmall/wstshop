...
class CronJobs extends Base{
	...
	public function autoCancelNoPay(){
		$autoCancelNoPayDays = (int)WSTConf('CONF.autoCancelNoPayDays');
	 	...
	 	$orders = Db::name('orders')->alias('o')->where('o.createTime<"'.$lastDay.'" and o.orderStatus=-2 and o.dataFlag=1 and o.payType=1 and o.isPay=0')->field("o.orderId,o.orderNo,o.userId")->select();
	 	if(!empty($orders)){
		    ...
		    try{
                ...
                foreach ($orders as $okey => $order){
                	$goods = Db::name('order_goods')->alias('og')->join('__GOODS__ g','og.goodsId=g.goodsId','inner')
					           ->where('orderId',$order['orderId'])->field('og.*')->select();
	...
	public function autoAppraise(){
        $autoAppraiseDays = (int)WSTConf('CONF.autoAppraiseDays');
	 	...
	 	$rs = model('orders')->where('receiveTime<"'.$lastDay.'" and orderStatus=2 and dataFlag=1 and isAppraise=0')->field("orderId,userId,orderScore,orderNo")->select();
	 	if(!empty($rs)){
	 		$prefix = config('database.prefix');
	 		$orderIds = [];
	...
	public function autoReceive(){
	 	...
	 	$rs = model('orders')->where('deliveryTime<"'.$lastDay.'" and orderStatus=1 and dataFlag=1')->field("orderId,orderNo,userId,orderScore")->select();
	 	if(!empty($rs)){
	 		$prefix = config('database.prefix');
	 		Db::startTrans();
		    try{
