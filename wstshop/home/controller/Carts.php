...
use wstshop\common\model\Carts as M;
/**
 * ============================================================================
 * WSTShop网上商店
...
class Carts extends Base{
    ...
    */
	...
	public function getCartMoney(){
		...
		$carts = $m->getCarts(true);
		$freight = 0;
		$data = ['total'=>0,'status'=>1];
		// 计算商品总价格
		$totalPrice = 0;
		foreach ($carts['carts'] as $key =>$v){
			$totalPrice += $v['shopPrice'];
		}
		// 如果该地区未设置运费则去默认运费
		$data['freight'] = (WSTOrderFreight($areaId)==0)?WSTConf('CONF.defaultFreight'):WSTOrderFreight($areaId);
		$data['total'] = $totalPrice+$data['freight'];
		return $data;
	}
	/**
	 * 修改购物车商品状态
