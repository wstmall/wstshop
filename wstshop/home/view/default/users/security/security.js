...
function vePayForm(){
	...
	myorm = $('#payform').validator({
          valid: function(form){
            ...
            var loading = WST.msg('正在提交数据，请稍后...', {icon: 'wstloading',time:60000});
...
function sendEmail(edit){
  ...
  var loading = WST.msg('正在发送邮件，请稍后...', {icon: 'wstloading',time:60000});
  var params = WST.getParams('.ipt');
...
function getPhoneVerifys(n){
	...
	$.post(WST.U('home/users/getPhoneVerify'+n),params,function(data,textStatus){
		...
		if(json.status!=1){
			...
			$('#timeObtain').html('获取手机验证码(120)').css('width','160px');
			var task = setInterval(function(){
				time--;
