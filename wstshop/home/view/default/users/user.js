...
function checkCoords()
{
  //判断是否有裁剪
  if (parseInt($('#w').val())){
    var loading = WST.msg('图像处理中，请稍候...', {icon: 'wstloading',shade: [0.3, '#999999']});
    /*获取裁剪数据*/
    var photoData = {};
    photoData.x = $('#x').val();
    photoData.y = $('#y').val();
    ...
    $.post(WST.U('Home/users/editUserPhoto'),photoData,function(data, textStatus){
      if(data.status==1)
      {
        ...
        //将上传的图片路径赋给全局变量
            userPic = data.data;
            $('#userPhotoPreview').html('<img id="userPhoto" class="ipt" src="'+WST.conf.ROOT+'/'+userPic+'?='+Math.random()+'"  style="width: 150px;height: 150px;border-radius: 50%;" />');
            $('#userPhotoPreview1').html('<img id="userPhoto1" class="ipt" src="'+WST.conf.ROOT+'/'+userPic+'?='+Math.random()+'" style="width: 150px;height: 150px;border-radius: 50%;" />');
              $('#userPhotoCut').hide();
              $('#userPhoto').show();
      }else{
  ...
  $('#userEditForm').validator({
        ...
        valid: function(form){
          ...
          $.post(WST.U('Home/Users/toEdit'),params,function(data,textStatus){
            ...
            if(json.status=='1'){
                WST.msg("操作成功",{icon:1},function(){history.go(0)});
                return;
            }else{
                  WST.msg(json.msg,{icon:2});
            }
